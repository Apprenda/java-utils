...
/**
 * A pooled {@link javax.sql.DataSource} implementation that extends {@link ApprendaAbstractDataSource}.
 * Underneath, it uses {@link org.apache.tomcat.jdbc.pool.ConnectionPool}.
 * <p/>
 * It implements {@link org.apache.tomcat.jdbc.pool.PoolConfiguration} to configure the
 * Tomcat JDBC Connection Pool.
 * These configuration properties can be set via {@code BeanProperties}.
 *
 */
public class ApprendaPoolDataSource extends ApprendaAbstractDataSource implements PoolConfiguration {
    ...
    public Connection getConnection(String username, String password) throws SQLException {
        ...
        if (datasource==null) {
            // if datasource is not set yet,
            // then create one, and
            // configure it with the propertyHolder
            propertyHolder.setUrl(jdbcUrl);
            propertyHolder.setDriverClassName(driverClass);
            propertyHolder.setUsername(user);
            propertyHolder.setPassword(pwd);
            this.datasource = new DataSource();
            datasource.setPoolProperties(this.propertyHolder);
        }
    ...
    private final PoolProperties propertyHolder = new PoolProperties();
